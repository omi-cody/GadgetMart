...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (userservice.checkDuplicacy(userID, StringUtil.CHECK_DUP_ID)
				|| userservice.checkDuplicacy(email, StringUtil.CHECK_DUP_EMAIL)
				|| userservice.checkDuplicacy(phoneNumber, StringUtil.CHECK_DUP_NUMBER)) {

			String errorMessage = StringUtil.DUPLICACY_NON;
			request.setAttribute(StringUtil.ERROR_VAL, errorMessage);
			request.getRequestDispatcher(StringUtil.REGISTER_PAGE).forward(request, response);
			return;
		}
